#include "../../include/Components/Xorl.hpp"

Xorl::Xorl(std::string type, std::string name)
{
    this->type = type;
    this->name = name;
...
void Xorl::simulate(std::size_t tick)
{
    std::size_t x = tick;
    tick = x;
    if (this->links.empty())
    ...
    if ((this->links.at(3).first != nullptr) && (this->links.at(4).first != nullptr) && (this->links.at(5).first != nullptr))
...
Nts::tristate Xorl::compute(std::size_t pin)
{
    return this->links.at(pin).first->getVal();
}

Nts::tristate Xorl::getVal()
{
    return this->display;
}

std::string Xorl::getName()
{
    return this->name;
}

std::string Xorl::getType()
{
    return this->type;
}

void Xorl::setVal(Nts::tristate val)
{
    this->val = val;
}
...
void Xorl::setLink(std::size_t pin, Nts::IComponent &other, std::size_t other_pin)
{
    if (pin > 14 || pin == 0) {
