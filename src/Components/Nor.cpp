#include "../../include/Components/Nor.hpp"

Nor::Nor(std::string type, std::string name)
{
    this->type = type;
    this->name = name;
...
Nts::tristate Nor::getVal()
{
    return this->display;
}

std::string Nor::getName()
{
    return this->name;
}

std::string Nor::getType()
{
    return this->type;
}

void Nor::setVal(Nts::tristate val)
{
    this->val = val;
}
...
void Nor::setLink(std::size_t pin, Nts::IComponent &other, std::size_t other_pin)
{
    if (pin > 14 || pin == 0) {
